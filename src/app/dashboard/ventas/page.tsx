...
export default function VentasPage() {
  ...
  const estadoBadge = (estado: string) => {
    switch (estado) {
      ...
      default: return "bg-[#2a2a2c] text-billar-gray";
    }
  };

  if (loading) {
    return (<div className="flex flex-col items-center justify-center py-24 text-billar-gray"><RefreshCw className="w-10 h-10 animate-spin text-billar-primary mb-4" /><p className="text-sm">Cargando historial de ventas...</p></div>);
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Receipt className="w-7 h-7 text-billar-primary" /> Historial de Ventas</h2>
          <p className="text-sm text-billar-gray">Revisa, filtra y gestiona todas las ventas registradas.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setIsDirectPOSOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-billar-primary hover:bg-billar-primary-dark text-white font-bold rounded-lg text-sm transition-all shadow-[0_0_15px_rgba(220, 38, 38,0.3)]"><Plus className="w-4 h-4" /> Venta Directa</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-[#1a1a1c] border border-[#2a2a2c] rounded-xl p-5">
          <div className="flex items-center gap-3"><div className="p-2.5 bg-billar-primary/10 rounded-xl"><ShoppingBag className="w-5 h-5 text-billar-primary" /></div><div><p className="text-xs text-billar-gray">Ventas Hoy</p><p className="text-2xl font-black text-white">{ventasHoy.length}</p></div></div>
        </div>
        <div className="bg-[#1a1a1c] border border-[#2a2a2c] rounded-xl p-5">
          <div className="flex items-center gap-3"><div className="p-2.5 bg-green-500/10 rounded-xl"><TrendingUp className="w-5 h-5 text-green-500" /></div><div><p className="text-xs text-billar-gray">Ingresos Hoy</p><p className="text-2xl font-black text-white">Bs. {ingresosHoy.toFixed(2)}</p></div></div>
        </div>
        <div className="bg-[#1a1a1c] border border-[#2a2a2c] rounded-xl p-5">
          <div className="flex items-center gap-3"><div className="p-2.5 bg-blue-500/10 rounded-xl"><CreditCard className="w-5 h-5 text-blue-400" /></div><div><p className="text-xs text-billar-gray">Método Más Usado</p><p className="text-2xl font-black text-white capitalize">{metodoTop}</p></div></div>
        </div>
      </div>

      {/* Filtros */}
      <div className="bg-[#1a1a1c] border border-[#2a2a2c] p-4 rounded-xl flex flex-wrap gap-3 items-center">
        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-billar-gray/50" /><input type="text" placeholder="Buscar por ID..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2 pl-10 pr-3 text-sm text-white w-44 focus:outline-none focus:border-billar-primary" /></div>
        <input type="date" value={fechaDesde} onChange={e => setFechaDesde(e.target.value)} className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-billar-primary" />
        <input type="date" value={fechaHasta} onChange={e => setFechaHasta(e.target.value)} className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-billar-primary" />
        <select value={filtroEstado} onChange={e => setFiltroEstado(e.target.value)} className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-billar-primary">
        ...
        <select value={filtroMetodo} onChange={e => setFiltroMetodo(e.target.value)} className="bg-black/40 border border-[#2a2a2c] rounded-lg py-2 px-3 text-sm text-white focus:outline-none focus:border-billar-primary">
              ...
              {filtered.length > 0 ? filtered.map(v => {
                ...
                return (
                  <tr key={v.id_venta} className="border-b border-[#2a2a2c]/40 hover:bg-white/[0.02] transition-colors text-sm text-white">
                    <td className="py-3 pl-6 text-xs"><div>{new Date(v.created_at).toLocaleDateString("es-BO")}</div><div className="text-billar-gray">{new Date(v.created_at).toLocaleTimeString("es-BO", { hour: "2-digit", minute: "2-digit" })}</div></td>
                    <td className="py-3">{mesaInfo ? `Mesa ${mesaInfo.numero}` : "—"}</td>
                    <td className="py-3 text-billar-gray text-xs">{v.usuarios?.nombre || "—"}</td>
                    ...
                    <td className="py-3 pr-6 text-center flex gap-2 justify-center">
                      <button onClick={() => setSelectedVenta(v)} className="p-1.5 hover:bg-[#2a2a2c] rounded-lg text-billar-gray hover:text-white transition-all" title="Ver detalle"><Eye className="w-4 h-4" /></button>
                      {v.estado === "completada" && <button onClick={() => handleAnular(v)} className="p-1.5 hover:bg-red-500/10 rounded-lg text-billar-gray hover:text-red-400 transition-all" title="Anular"><XCircle className="w-4 h-4" /></button>}
                    </td>
                  </tr>
                );
              }) : (
                <tr><td colSpan={8} className="py-12 text-center text-billar-gray text-sm">No hay ventas registradas con estos filtros.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#1a1a1c] border border-[#2a2a2c] w-full max-w-lg rounded-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-[#2a2a2c] flex justify-between items-center">
              ...
              <button onClick={() => setSelectedVenta(null)} className="p-2 hover:bg-[#2a2a2c] rounded-full text-billar-gray"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-billar-gray text-xs block">Fecha</span><span className="text-white font-bold">{new Date(selectedVenta.created_at).toLocaleString("es-BO")}</span></div>
                <div><span className="text-billar-gray text-xs block">Estado</span><span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${estadoBadge(selectedVenta.estado)}`}>{selectedVenta.estado}</span></div>
                <div><span className="text-billar-gray text-xs block">Método de Pago</span><span className="text-white font-bold capitalize">{selectedVenta.metodo_pago}</span></div>
                <div><span className="text-billar-gray text-xs block">Atendió</span><span className="text-white font-bold">{selectedVenta.usuarios?.nombre || "—"}</span></div>
              </div>
              <div className="border-t border-[#2a2a2c] pt-4">
                <h4 className="text-xs font-bold text-billar-gray uppercase tracking-wider mb-3">Productos</h4>
                <div className="space-y-2">
                  {selectedVenta.venta_items?.map((item: any, i: number) => (
                    <div key={i} className="flex justify-between items-center bg-black/30 p-3 rounded-lg text-sm">
                      <div><span className="text-white font-bold">{item.productos?.nombre || "Producto"}</span><span className="text-billar-gray text-xs ml-2">x{Number(item.cantidad).toFixed(0)}</span></div>
                      <span className="font-bold text-white">Bs. {Number(item.subtotal).toFixed(2)}</span>
                    </div>
                  ))}
                  {(!selectedVenta.venta_items || selectedVenta.venta_items.length === 0) && <p className="text-billar-gray text-sm text-center py-4">Solo tiempo de juego (sin productos adicionales)</p>}
                </div>
              </div>
              <div className="bg-billar-primary/10 border border-billar-primary/30 p-4 rounded-xl flex justify-between items-center">
                <span className="text-sm text-billar-primary font-bold uppercase">Total</span>
